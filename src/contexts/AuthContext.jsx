...
export const AuthProvider = ({ children }) => {
  ...
  const registerProvider = async ({ email, password, firstName, lastName, username, phone, description, categories, location, address, priceRange }) => {
    const { user } = await createUserWithEmailAndPassword(auth, email, password);
    const profile = {
      uid: user.uid, email, firstName, lastName,
      username: username || email.split('@')[0],
    ...
    const providerData = {
      ...
      profileImageUrl: '', rating: 0, reviewCount: 0,
  ...
  const loginWithGoogle = async (role = 'customer') => {
    ...
    const profile = {
      uid: user.uid, email: user.email, firstName, lastName,
      username: user.email.split('@')[0],
      phone: '', address: '', role,
      createdAt: serverTimestamp(), favorites: [],
  ...
  const value = {
    ...
    registerCustomer, registerProvider, resetPassword,
  };
