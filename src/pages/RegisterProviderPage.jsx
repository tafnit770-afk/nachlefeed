...
import { User, Mail, Lock, Phone, MapPin, AtSign, DollarSign, FileText, Search, X, Check, Eye, EyeOff } from 'lucide-react';
...
export default function RegisterProviderPage() {
  const { registerProvider, loginWithGoogle } = useAuth();
  ...
  return (
    <div className="auth-page">
      <div className="auth-card" style={{ maxWidth: 580 }}>
        ...
        {/* Google Button */}
        <button className="btn-google" type="button" disabled={googleLoading}
          onClick={async () => {
            setGoogleLoading(true);
            try {
              await loginWithGoogle('provider');
              toast.success('נרשמת כספק עם Google! השלם את הפרופיל שלך 🎉');
              navigate('/provider-dashboard');
            } catch { toast.error('שגיאה בהרשמה עם Google'); }
        ...
        <div className="auth-divider"><span>או מלא פרטים</span></div>
          ...
          <div className="form-grid-2">
            <div className="input-group"><label>סיסמה * (לפחות 6 תווים)</label>
              <div className="input-icon-wrapper">
                ...
                <input type={showPassword ? 'text' : 'password'} className="input-field input-with-icon" style={{ paddingLeft: 38 }} placeholder="••••••••" value={form.password} onChange={e => update('password', e.target.value)} required />
